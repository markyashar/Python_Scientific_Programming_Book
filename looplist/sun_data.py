...
# In the lines above, we've initialized the data by copying the data from the Oxford_sun_hours.txt
# file into the program file and set a variable name on the left hand side.

# Task 1: We need to establish a list 'monthly_mean' w/results from the computation,i.e.,
# monthly_mean[2] holds average # of sun hours for March in period 1929-2009. Average is computed
# in standard way: for each month, we run through all the years, sum up the values, & finally 
# divide by # of yrs. (2009-1929+1).   When looping over yrs. & months it's convenient to have loop
# variables running over the true yrs. (1929-2009) & standard month # (1-12). These variables must be
# correctly translated to indices in the 'data' list such that all indices start at 0. 
monthly_mean = [0]*12     # list w/12 elements.
#
# An alternative solution would be to introduce separate variables for the monthly averages, say
# Jan_mean, Feb_mean, etc. Reader can as an exercise write code associated w/such solution and
# realize that using monthly_mean list is more elegant and yields much simpler & shorter code. 
# Separate variables might be ok solution for 2-3 variables, but not for as many as 12.
for month in range(1, 13):
    ...
    for year in range(1929, 2010):
        ...
        s += data[y][m]
    monthly_mean[m] = s/n
# Perhaps we want nice-looking printout of the results. This can elegantly be created by 1st 
# defining a tuple (or list) of the names of the months & then running through this list in parallel
# w/montly_mean
...
# Task 2: This task can be solved by pure inspection of above printout, which reveals that June
# is winner. However, since we're learning programming, should be able to replace our eyes w/some
# computer code to automate task. Maximum value 'max_value' of list like 'montly_mean' is simply
# obtained by 'max(monthly_mean). Corresponding index, needed to find the right name of corresponding
...
print '%s has best weather with %.1f sun hours on average' % \
      (month, max_value)
# Instead of using Python's 'max' and 'index' functionality, we could code everything ourselves...
max_value = -1E+20
...
# Task 3: This task requires us to 1st develop an algorithm for how to compute the decade averages.
# The algorithm, expressed w/words, goes as follows: We loop over the decades, & for each decade,
# we loop over its years, & for each year, we add the December data of previous year and January
# data of current year to an accumulation variable. Dividing this accumulation variable by 10*2*30
# gives average # of sun hours per day in winter time for particular decade.
decade_mean = []
...
# convert to h, min, sec:
"""
 import datetime
 for i in range(len(decade_mean)):
     decade_mean[i] = str(datetime.timedelta(seconds=decade_mean[i]*3600))
 for i in range(len(decade_mean)):
    print 'Decade %d-%d: %s' % \
          (1930+i*10, 1939+i*10, decade_mean[i])
"""
# alternative:
for i in range(len(decade_mean)):
    m, s = divmod(decade_mean[i]*3600, 60)
    h, m = divmod(m, 60)
    decade_mean[i] = (h, m, s)

for i in range(len(decade_mean)):
    print 'Decade %d-%d: %dh %dm' % \
          (1930+i*10, 1939+i*10, decade_mean[i][0], decade_mean[i][1])

"""
Problem: The file src/misc/Oxford_sun_hours.txt contains data of # of sun hours in Oxford, UK, for
every month since Jan. 1929. Data are already on suitable nested list format: 
...
The list in every line holds # of sun hours for each of year's 12 months, i.e., 1st index in nested
list corresponds to year and 2nd index corresponds to month #. More precisely, the double index
[i][j] corresponds to year 1929+i and month 1+j (January being month # 1). 
   Task is to define this nested list in a program and do following data analysis:
(1) Compute average # of sun hours for each month during total data period (1929-2009).
(2) Which month has best weather according to means found in preceding task?
(3) For each decade, 1930-1939,1940-1949,...,2000-2009, compute average # of sun hours per day in
Jan. and Dec. For example, use Dec. 1949, Jan. 1950,..., Dec. 1958, and Jan. 1959 as data for the
decade 1950-1959. Are there any noticeable differences between the decades?

Overall output from the code is as follows:
...
Decade 1990-1999: 1.8
Decade 2000-2009: 2.1
Decade 1930-1939: 1h 41m
Decade 1940-1949: 1h 45m
Decade 1950-1959: 1h 49m
Decade 1960-1969: 1h 49m
Decade 1970-1979: 1h 34m
Decade 1980-1989: 2h 0m
Decade 1990-1999: 1h 45m
Decade 2000-2009: 2h 6m
